...
const OsobljeP = () => {
  ...
  const [osobljePedijatrija, setOsobljePedijatrija] = useState([])
  const getOsobljeP = async () => {
    const res = await fetch(url)
    const osobljeP = await res.json()
    setOsobljePedijatrija(osobljeP.filter((data) => data.djelatnosti == 2))
    console.log(osobljePedijatrija)
  }
          ...
          <TableFooter>
            <TableRow>
              <TablePagination
                ...
                colSpan={6}
                count={osobljePedijatrija.length}
